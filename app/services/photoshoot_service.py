...
_REALISM_PROMPT = (
    ...
    "Do NOT change composition, pose, background geometry, or facial identity (bone structure, feature shapes/sizes, "
    "expression, gaze, moles/freckles, hair style).\n"
    "PRIORITY-1 (NON-NEGOTIABLE): If skin looks airbrushed, poreless, waxy, or uniform, you MUST add visible micro-pores, "
    "fine grain, subsurface veins, and natural specular micro-highlights\u2014that is required realism, NOT an identity change.\n"
    "\n"
    "=== HARD CONSTRAINTS \u2014 NEVER CHANGE THESE (only add surface/lighting micro-detail ON the existing subject) ===\n"
    "- FACE IDENTITY: Same person as IMG1. Do NOT change face shape, bone structure, feature size/position/spacing, "
    "ethnicity read, age, eye size/openness, nose width/length, lip fullness, jaw/chin line, brow height, ear shape, or "
    "expression/gaze beyond rendering existing skin more realistically. Skin edits are SURFACE ONLY (pores, veins, grain, "
    "tiny speculars on existing skin)\u2014never cosmetic reshaping or beautification that alters who they are.\n"
    "- GARMENT: Same exact outfit as IMG1\u2014same garment pieces, colors, print/pattern placement, embroidery, cut, buttons, "
    "hem, drape silhouette, accessories. Do NOT swap, add, remove, recolor, or reinterpret the clothing.\n"
    "- BACKGROUND: Do NOT repaint, replace, extend, crop, or alter the environment. Same architecture, floor, sky, props, "
    "colors, and layout as IMG1. All scene edits are limited to how the subject\u2019s edges and light response sit against "
    "that unchanged background (no new bg elements).\n"
    "- POSE: IDENTICAL to IMG1\u2014same limb angles, torso orientation, head tilt, hand/finger placement, foot placement, "
    "and framing. Do NOT move, rotate, rescale, or reposition the person in the frame; do not change gesture or stance.\n"
    "\n"
    ...
    "- SKIN TEXTURE (additive only): Intensify DSLR-like micro-pores on nose, inner cheeks, forehead, and chin so "
    "they read at NORMAL viewing size (not microscopic). Add fine irregular grain, tiny specular hits on nose bridge and "
    "cheek peaks from the SAME light direction as the scene, and subtle blue-green venous hints on temples/sides of face "
    "and neck where real skin would show them under sidelight. Hands/arms/legs: veins and tendons must read clearly. "
    "Do NOT alter facial structure or proportions to do this\u2014only surface micro-detail. Plastic or featureless skin means failure.\n"
    "\n"
    "[BODY & POSE LOCK]\n"
    ...
    "- You MAY add scene-correct lighting RESPONSE only (how light hits existing fabric): directional highlights and "
    "micro-shadows along existing weave, embroidery, folds, and lace; subtle bounce tint\u2014without changing base color, "
    "pattern, print registration, or garment shape.\n"
    "\n"
    ...
    "- MICRO-PORES (MUST BE VISIBLE): Irregular pore field on nose, inner cheeks, forehead, chin. Each pore: tiny shadowed "
    "well + soft rim highlight from scene light\u2014not a flat uniform dot grid. T-zone slightly oilier sheen; do NOT blur "
    "or melt pores away.\n"
    "- MICRO-CONTRAST: Add fine high-frequency grain on cheeks/jaw/forehead (sensor+skin texture), not noise blobs. "
    "Break up large smooth skin patches.\n"
    "- VEINS (READABLE, ANATOMICAL): Hands/forearms/wrists: clearly visible branching blue-green veins and tendons. "
    "Neck/temples/side of face: subtle but real venous map where sidelight would reveal it\u2014soft, under-skin, tapered; "
    "must react to light (not tattoo-like).\n"
    "- SPECULAR / OIL: Small natural skin highlights on nose bridge, cheekbones, Cupid's bow, and shoulders that MATCH the "
    "scene's key light color and direction\u2014breaks the dead matte AI look.\n"
    "- PERIORBITAL + CHEEK: Fine micro-creases and micro-grain under eyes and smile zones (light, not deep wrinkles unless "
    "already in source).\n"
    ...
    "- Specular highlights on strands from the scene key + fill (warm/cool matching environment), not flat global sheen. "
    "Rim/backlight on hair mass where background is bright; secondary bounce tint from walls/sky onto dark hair.\n"
    ...
    "Make the existing placement READ MORE grounded WITHOUT changing pose: refine contact shadows, AO at soles, and edge "
    "integration so feet read on the existing floor plane at the SAME pixel position as IMG1.\n"
    ...
    "Background in IMG1 is FIXED\u2014do not edit background pixels, colors, or objects. Only adjust how the SUBJECT "
    "receives light so it matches the existing scene:\n"
    "- Match key light direction, color temperature, and shadow hardness on face, garment, and hair to the environment "
    "(e.g. warm sun from alley opening, cool sky fill, colored walls).\n"
    "- COLOR BOUNCE / GI: Add believable reflected light from ground, buildings, flowers, sky onto lower face, chin, "
    "undersides of arms, garment folds, and hair\u2014same hues as nearby surfaces, low intensity.\n"
    "- Rim and edge light on hair, shoulders, and ear rims from bright areas behind/side of subject; soft light wrap on "
    "silhouette against bright bg.\n"
    "- Ground contact shadow with soft penumbra at feet; darker ambient occlusion where sole meets ground. Shoes cast "
    "shadow onto floor consistent with scene light.\n"
    ...
    "- EDGE BLENDING: Seamless boundary; no razor cutout; no aliasing or color fringe.\n"
    "\n"
    ...
    "- RECHECK: Output must still show the SAME face identity, SAME garment, SAME pose, SAME background as IMG1\u2014only "
    "surface realism and scene-consistent light on the subject improved."
)
